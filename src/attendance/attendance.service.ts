  ...
  export class AttendanceService {
    ...
    private async handlePunchOutUpdate(createAttendanceDto: CreateAttendanceDto) {
      const result = await this.prisma.$transaction(async (prisma) => {
        ...
        const workHours = (punchOutTime.getTime() - punchInTime.getTime()) / (1000 * 60 * 60); // Convert to hours
        ...
        await prisma.userPunch.update({
          ...
          data: {
            ...
            overtime: createAttendanceDto.overtime || 0,
        ...
        // Also create a separate punch out record for tracking
              await prisma.userPunch.create({
                data: {
                  attendanceId: createAttendanceDto.attendanceId!,
                  punchIn: punchOutTime,
    ...
    private async updateDailyWorkHours(attendanceId?: string) {
      if (!attendanceId) {
        // Nothing to update if attendanceId is not provided
        return;
      }

      const punches = await this.prisma.userPunch.findMany({
        where: { 
          attendanceId,
          punchOut: { not: null },
          punchType: PunchType.IN
      ...
      const totalWorkHours = punches.reduce((sum, punch) => sum + (punch.workHours || 0), 0);
